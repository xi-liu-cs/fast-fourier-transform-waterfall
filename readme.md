...
random number generator}\\
  \mathbf{d}(\mathbf{x}, t) = \sum -i \frac{\mathbf{k}}{k} \widetilde{h}(\mathbf{k}, t) e ^ {i\mathbf{k} \cdot x} \quad \text{the 2d displacement vector field is computed using the fourier amplitudes of the height field}\\
}
```
